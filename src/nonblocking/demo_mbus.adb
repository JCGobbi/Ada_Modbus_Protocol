...
begin
   ...
   Serial_Timer.Configure_Timeout (MBus_TIM.Device.all, MB_Bps => MBus_Bps);
   ...
   loop
      ...
      then
         ...
         --  Instead of using the MBus_Task, here we receive only one frame.
         --  If your board is connected to a modbus client that responds to the
         --  MBus_Read_Discrete_Inputs, then you may uncomment the
         --  "Receive_Frame (MBus_COM);" bellow. But then you need to comment
         --  the "Start_Timeout" procedure bellow.

         --  Receive_Frame (MBus_COM);

         --  When the first byte is received, the timeouts are started but, if
         --  you don't have a modbus client, you may test the return of timeout
         --  with the "Start_Timeout" procedure bellow.
         Serial_Timer.Start_Timeout (MBus_COM.Timer_Periph.all,
                                     Mode => Get_Serial_Mode (MBus_COM));
         ...
         --  Wait for the end of frame in a regular frame reception to the
         --  incoming message buffer or inter-frame/response timeout.
