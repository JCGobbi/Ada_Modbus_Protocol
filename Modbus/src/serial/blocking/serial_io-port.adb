...
package body Serial_IO.Port is
   ...
   is
   begin
      for Next in 1 .. Get_Length (This.Outgoing_Msg.all) loop
         Await_Send_Ready (This.Device.Transceiver.all);
         Transmit (This.Device.Transceiver.all,
                   UInt9 (Get_Content_At (This.Outgoing_Msg.all, Next)));
      end loop;
      Signal_Transmission_Complete (This.Outgoing_Msg.all);
   end Send;
   ...
   is
      ...
      Timed_Out : Boolean;
   begin
      Clear (This.Incoming_Msg.all);
      Receiving : for K in 1 .. This.Incoming_Msg.all.Physical_Size loop
         ...
         case This.Serial_Mode is
            when MBus_RTU =>
               ...
               if Timed_Out then -- Reached maximum inter-character time
                  ...
                  MBus_Note_Error (This.Incoming_Msg.all, InterChar_Timed_Out);
                  ...
                  if Timed_Out then --  Reached maximum inter-frame time
                     ...
                     MBus_Note_Error (This.Incoming_Msg.all, InterFrame_Timed_Out);
                     ...
                     if Timed_Out then --  Reached maximum response time
                        ...
                        MBus_Note_Error (This.Incoming_Msg.all, Response_Timed_Out);
                     end if;
                     exit Receiving;
                  end if;
               end if;
         ...
         if Rx_Ready (This.Device.Transceiver.all) then
            ...
            case This.Serial_Mode is
               when MBus_RTU =>
                  Append (This.Incoming_Msg.all, UInt8 (Raw));
               when MBus_ASCII =>
                  Append (This.Incoming_Msg.all, UInt8 (Raw));
                  --  Test end-frame sequence CR + LF for modbus ASCII.
                  if Get_Length (This.Incoming_Msg.all) > 1 then
                     if Get_Content_At (This.Incoming_Msg.all,
                                        Get_Length (This.Incoming_Msg.all)) = 16#0A# and -- LF character
                        Get_Content_At (This.Incoming_Msg.all,
                                        Get_Length (This.Incoming_Msg.all) - 1) = 16#0D# -- CR character
                     then
                        exit Receiving;
                     end if;
                  end if;
               when Terminal =>
                  ...
                  Append (This.Incoming_Msg.all, UInt8 (Raw));
                  exit Receiving when UInt8 (Raw) =
                    Character'Pos (Get_Terminator (This.Incoming_Msg.all));
            end case;
            Clear_Status (This.Device.Transceiver.all, Read_Data_Register_Not_Empty);
         end if;

      end loop Receiving;
      Signal_Reception_Complete (This.Incoming_Msg.all); -- incoming buffer
   end Receive;
